...
5. **Final Depth Interpolation & Uncertainty Display:** Performs spatial Kriging interpolation on the products <i>P</i><sub>D,i</sub> = <i>D</i><sub>i</sub> sin(<i>α</i><sub>opt,i</sub>) to reconstruct continuous depth <i>D</i>(<i>x</i>,<i>y</i>) and bedrock elevation <i>Z</i>(<i>x</i>,<i>y</i>) fields. The Kriging standard error for the interpolated depths <i>σ</i><sub>D</sub>(<i>x</i>,<i>y</i>) is converted to ±meters to quantify depth uncertainty.
